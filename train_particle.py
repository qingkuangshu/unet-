...
if __name__ == "__main__":    
    ...
    #----------------------------------------------------------------------------------------------------------------------------#
    model_path      = "E://unet//unet-keras-master -particle - test//model_data//vgg16_weights_tf_dim_ordering_tf_kernels_notop.h5"
                           
    #------------------------------#
    #   输入图片的大小E:\unet\unet-keras-master -particle - test\unet-keras-master -particle - test\unet-keras-master\
    #------------------------------#
    input_shape     = [512, 512]
    ...
    loss_history    = LossHistory('logs/', val_loss_flag = True)
    ...
    if True:
        ...
        train_dataloader    = UnetDataset(train_lines, input_shape, batch_size, num_classes, True, VOCdevkit_path)
        validation_dataloader    = UnetDataset(validation_lines, input_shape, 1, num_classes, False, VOCdevkit_path)
        print('Train on {} samples, with batch size {}.'.format(len(train_lines), batch_size))
        model.fit_generator(
            generator           = train_dataloader,
            steps_per_epoch     = epoch_step,
